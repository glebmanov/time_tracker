defmodule TimeTrackerWeb.TaskLive.FormComponent do
  ...
  def update(%{task: task} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(Tracker.change_task(task))
     end)}
  end

  @impl true
  def handle_event("validate", %{"task" => task_params}, socket) do
    changeset = Tracker.change_task(socket.assigns.task, task_params)
  ...
  defp save_task(socket, :edit, task_params) do
    case Tracker.update_task(socket.assigns.task, task_params) do
      {:ok, task} ->
